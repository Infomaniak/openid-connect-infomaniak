...
/**
 * OpenID_Connect_Infomaniak_Client class.
 *
 * Plugin OIDC/oAuth client class.
 *
 * @package  OpenID_Connect_Infomaniak
 * @category Authentication
 */
class OpenID_Connect_Infomaniak_Client {

	/**
	 * The OIDC/oAuth client ID.
	 *
	 * @see OpenID_Connect_Infomaniak_Option_Settings::client_id
	 *
	 * @var string
	 */
	private $client_id;
	 ...
	 *
	 * @see OpenID_Connect_Infomaniak_Option_Settings::client_secret
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_Infomaniak_Option_Settings::scope
	 *
	 * @var string
	 */
	private $scope;
	 ...
	 *
	 * @see OpenID_Connect_Infomaniak_Option_Settings::endpoint_login
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_Infomaniak_Option_Settings::endpoint_userinfo
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_Infomaniak_Option_Settings::endpoint_token
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_Infomaniak_Option_Settings::redirect_uri
	 *
	 * @var string
	 */
	 ...
	 *
	 * @see OpenID_Connect_Infomaniak_Option_Settings::acr_values
	 *
	 * @var string
	 */
	private $acr_values;
	 ...
	 *
	 * @see OpenID_Connect_Infomaniak_Option_Settings::state_time_limit
	 *
	 * @var int
	 */
	 ...
	 * Client constructor.
	 *
	 * @param string                               $client_id          @see OpenID_Connect_Infomaniak_Option_Settings::client_id for description.
	 * @param string                               $client_secret      @see OpenID_Connect_Infomaniak_Option_Settings::client_secret for description.
	 * @param string                               $scope              @see OpenID_Connect_Infomaniak_Option_Settings::scope for description.
	 * @param string                               $endpoint_login     @see OpenID_Connect_Infomaniak_Option_Settings::endpoint_login for description.
	 * @param string                               $endpoint_userinfo  @see OpenID_Connect_Infomaniak_Option_Settings::endpoint_userinfo for description.
	 * @param string                               $endpoint_token     @see OpenID_Connect_Infomaniak_Option_Settings::endpoint_token for description.
	 * @param string                               $redirect_uri       @see OpenID_Connect_Infomaniak_Option_Settings::redirect_uri for description.
	 * @param string                               $acr_values         @see OpenID_Connect_Infomaniak_Option_Settings::acr_values for description.
	 * @param string                               $endpoint_jwks      The JWKS endpoint URL for JWT signature verification.
	 * @param string                               $issuer             The expected issuer for JWT validation.
	 * @param int                                  $jwks_cache_ttl     The JWKS cache TTL in seconds.
	 * @param bool                                 $allow_internal_idp Allow internal/private network endpoints.
	 * @param int                                  $state_time_limit   @see OpenID_Connect_Infomaniak_Option_Settings::state_time_limit for description.
	 * @param OpenID_Connect_Infomaniak_Option_Logger $logger             The plugin logging object instance.
	 */
	public function __construct( $client_id, $client_secret, $scope, $endpoint_login, $endpoint_userinfo, $endpoint_token, $redirect_uri, $acr_values, $endpoint_jwks, $issuer, $jwks_cache_ttl, $allow_internal_idp, $state_time_limit, $logger ) {
	 ...
	 *
	 * By default, uses wp_safe_remote_post() which blocks requests to
	 * internal/private networks (SSRF protection). If allow_internal_idp is
	 * enabled, uses wp_remote_post() to allow connections to localhost and
	 * private network identity providers.
	 *
	...
	public function validate_authentication_request( $request ) {
		...
		if ( ! isset( $request['state'] ) ) {
			do_action( 'infomaniak-connect-openid-no-state-provided' );
			return new WP_Error( 'missing-state', __( 'Missing state.', 'infomaniak-connect-openid' ), $request );
		}

		if ( ! $this->check_state( $request['state'] ) ) {
			return new WP_Error( 'invalid-state', __( 'Invalid state.', 'infomaniak-connect-openid' ), $request );
		}
	...
	public function get_authentication_code( $request ) {
		if ( ! isset( $request['code'] ) ) {
			return new WP_Error( 'missing-authentication-code', __( 'Missing authentication code.', 'infomaniak-connect-openid' ), $request );
		}
	...
	public function request_authentication_token( $code ) {
		...
		$request = apply_filters( 'infomaniak-connect-openid-alter-request', $request, 'get-authentication-token' );
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'request_authentication_token', __( 'Request for authentication token failed.', 'infomaniak-connect-openid' ) );
		}
	...
	public function request_new_tokens( $refresh_token ) {
		...
		// Allow modifications to the request.
		$request = apply_filters( 'infomaniak-connect-openid-alter-request', $request, 'refresh-token' );
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'refresh_token', __( 'Refresh token failed.', 'infomaniak-connect-openid' ) );
		}
	...
	public function get_token_response( $token_result ) {
		if ( ! isset( $token_result['body'] ) ) {
			return new WP_Error( 'missing-token-body', __( 'Missing token body.', 'infomaniak-connect-openid' ), $token_result );
		}
		...
		if ( is_null( $token_response ) ) {
			return new WP_Error( 'invalid-token', __( 'Invalid token.', 'infomaniak-connect-openid' ), $token_result );
		}
	...
	public function request_userinfo( $access_token ) {
		// Allow modifications to the request.
		$request = apply_filters( 'infomaniak-connect-openid-alter-request', array(), 'get-userinfo' );
		...
		if ( is_wp_error( $response ) ) {
			$response->add( 'request_userinfo', __( 'Request for userinfo failed.', 'infomaniak-connect-openid' ) );
		}
	...
	public function check_state( $state ) {
		...
		if ( ! get_option( '_transient_infomaniak-connect-openid-state--' . $state ) ) {
			do_action( 'infomaniak-connect-openid-state-not-found', $state );
			$state_found = false;
		}

		$valid = get_transient( 'infomaniak-connect-openid-state--' . $state );

		if ( ! $valid && $state_found ) {
			do_action( 'infomaniak-connect-openid-state-expired', $state );
		}
	...
	public function get_authentication_state( $request ) {
		if ( ! isset( $request['state'] ) ) {
			return new WP_Error( 'missing-authentication-state', __( 'Missing authentication state.', 'infomaniak-connect-openid' ), $request );
		}
	...
	public function get_id_token_claim( $token_response ) {
		...
		if ( ! isset( $token_response['id_token'] ) ) {
			return new WP_Error( 'no-identity-token', __( 'No identity token.', 'infomaniak-connect-openid' ), $token_response );
		}
	...
	public function validate_id_token_claim( $id_token_claim ) {
		...
		if ( ! isset( $id_token_claim['sub'] ) || empty( $id_token_claim['sub'] ) ) {
			return new WP_Error( 'no-subject-identity', __( 'No subject identity.', 'infomaniak-connect-openid' ), $id_token_claim );
		}

		// Validate expiration claim.
		if ( ! isset( $id_token_claim['exp'] ) ) {
			return new WP_Error( 'missing-exp', __( 'Token missing expiration claim.', 'infomaniak-connect-openid' ), $id_token_claim );
		}
		if ( time() >= (int) $id_token_claim['exp'] ) {
			return new WP_Error( 'token-expired', __( 'Token has expired.', 'infomaniak-connect-openid' ), $id_token_claim );
		}

		// Validate issued at claim.
		if ( ! isset( $id_token_claim['iat'] ) ) {
			return new WP_Error( 'missing-iat', __( 'Token missing issued at claim.', 'infomaniak-connect-openid' ), $id_token_claim );
		}

		// Validate audience claim matches client_id (can be string or array).
		if ( ! isset( $id_token_claim['aud'] ) ) {
			return new WP_Error( 'missing-aud', __( 'Token missing audience claim.', 'infomaniak-connect-openid' ), $id_token_claim );
		}
		...
		if ( ! $audience_valid ) {
			return new WP_Error( 'invalid-aud', __( 'Token audience does not match client.', 'infomaniak-connect-openid' ), $id_token_claim );
		}
		...
		if ( ! empty( $expected_issuer ) ) {
			if ( ! isset( $id_token_claim['iss'] ) ) {
				return new WP_Error( 'missing-iss', __( 'Token missing issuer claim.', 'infomaniak-connect-openid' ), $id_token_claim );
			}
		...
		if ( ! empty( $this->acr_values ) && isset( $id_token_claim['acr'] ) ) {
			if ( $this->acr_values != $id_token_claim['acr'] ) {
				return new WP_Error( 'no-match-acr', __( 'No matching acr values.', 'infomaniak-connect-openid' ), $id_token_claim );
			}
		}
	...
	public function get_user_claim( $token_response ) {
		...
		if ( is_wp_error( $user_claim_result ) || ! isset( $user_claim_result['body'] ) ) {
			return new WP_Error( 'bad-claim', __( 'Bad user claim.', 'infomaniak-connect-openid' ), $user_claim_result );
		}
	...
	public function validate_user_claim( $user_claim, $id_token_claim ) {
		...
		if ( ! is_array( $user_claim ) ) {
			return new WP_Error( 'invalid-user-claim', __( 'Invalid user claim.', 'infomaniak-connect-openid' ), $user_claim );
		}

		// Allow for errors from the IDP.
		if ( isset( $user_claim['error'] ) ) {
			$message = __( 'Error from the IDP.', 'infomaniak-connect-openid' );
		...
		if ( $id_token_claim['sub'] !== $user_claim['sub'] ) {
			return new WP_Error( 'incorrect-user-claim', __( 'Incorrect user claim.', 'infomaniak-connect-openid' ), func_get_args() );
		}

		// Allow for other plugins to alter the login success.
		$login_user = apply_filters( 'infomaniak-connect-openid-user-login-test', true, $user_claim );

		if ( ! $login_user ) {
			return new WP_Error( 'unauthorized', __( 'Unauthorized access.', 'infomaniak-connect-openid' ), $login_user );
		}
